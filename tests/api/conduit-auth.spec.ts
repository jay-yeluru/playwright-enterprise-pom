...
import { apiTags, dashboardTags, REGRESSION } from '../../constants/tags';
import config from '../../utils/config';

test.describe(`Conduit API: Authentication ${apiTags.CONDUIT} ${apiTags.API}`, () => {

    test(`should login successfully with valid credentials ${apiTags.SMOKE} ${dashboardTags.AUTH}`, async ({ app, utils, data }) => {
    ...
    test(`should return 401 for login with invalid password ${REGRESSION} ${dashboardTags.AUTH}`, async ({ app, utils, data }) => {
        const apiUtils = utils.apiUtils;
        const conduitApi = app.getConduitApi();
        const user = data.createUser();
